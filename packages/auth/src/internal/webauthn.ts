import type { JupiterAuth } from '../jupiterAuth'
import type {
...
import { base64UrlToUint8Array, bytesToBase64URL } from './base64'
...
export class WebAuthnApi {
  ...
  constructor(private client: JupiterAuth) {
          ...
          .then((factors) =>
            factors.data?.all.find(
              (v) =>
                ...
                v.display_name === friendlyName &&
                v.status !== 'unverified'
            )
          )
          .then((factor) =>
            factor?.id ? this.client.mfa.unenroll({ factorId: factor.id }) : void 0
          )
